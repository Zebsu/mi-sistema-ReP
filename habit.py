...
with st.sidebar:
    ...
    with tab_prac:
        if not st.session_state.data["lineas"]:
            ...
            p_hora = st.time_input("Hora", key="add_p_hora")
            ...
            if st.button("Guardar Práctica", use_container_width=True, key="btn_guardar_nueva"):
                if p_nombre and len(p_dias) > 0:
                    ...
                    nueva = {
                        ...
                        "time": p_hora.strftime("%H:%M"), "days": p_dias, "desc": p_desc,
                ...
                with st.expander(f"✏️ {p['name']}"):
                    ...
                    if c_g.button("Guardar", key=f"e_save_{p['id']}", use_container_width=True):
                        st.session_state.data["practicas"][i].update({
                            "name": e_nombre, "linea": e_linea, "time": e_hora.strftime("%H:%M"),
...
with col_der:
    with st.expander("🧭 Navegación Principal", expanded=True):
        vista = st.radio(
            "Ir a:", 
...
with col_principal:
    ...
    # === VISTA: REVISIÓN SEMANAL ===
        ...
        for rol in st.session_state.data["roles"]:
            if rol.get("histeric_list"):
                hay_pendientes = True
                st.markdown(f"**Rol: {rol['name']}**")
                for idx, item in enumerate(rol["histeric_list"]):
                    c_it1, c_it2 = st.columns([4, 1.5])
                    c_it1.write(f"- {item['text']}")
                    if c_it2.button("Crear Práctica", key=f"h_to_p_{rol['id']}_{item['id']}", use_container_width=True):
                        st.session_state["draft_practice_name"] = item['text']
                        st.session_state.data["roles"][next(i for i, r in enumerate(st.session_state.data["roles"]) if r["id"] == rol["id"])]["histeric_list"].pop(idx)
                        guardar_datos(st.session_state.data)
        ...
        for semana in cal:
            ...
            for i, dia in enumerate(semana):
                if dia != 0:
                    ...
                    label = f"📍 Hoy • {p_dia}pts" if fecha_str == str_hoy else f"✅ {dia} • {p_dia}pts" if p_dia > 0 else f"{dia}"
                    if cols[i].button(label, key=f"c_{fecha_str}", use_container_width=True): st.session_state.selected_cal_date = fecha_str
                else: cols[i].write("") 
    ...
    # === VISTA: ROLES (REDISEÑO) ===
        ...
        with tab_crear_rol:
            ...
            n_rol = c_r1.text_input("Nombre del Rol", placeholder="Ej. Ingeniero Core IP", key="n_rol_input")
        ...
        with tab_mis_roles:
            if not st.session_state.data["roles"]:
                ...
                idx = next(i for i, r in enumerate(st.session_state.data["roles"]) if r["name"] == rol_sel)
                rol = st.session_state.data["roles"][idx]
                ...
                with st.container():
                    ...
                    if not rol.get("objective", "") or st.session_state[edit_key]:
                        ...
                        if c_btn1.button("Guardar Objetivo", key=f"btn_save_obj_{rol['id']}", type="primary", use_container_width=True): 
                            st.session_state.data["roles"][idx]["objective"] = obj
                            st.session_state[edit_key] = False
                            guardar_datos(st.session_state.data)
                            st.rerun()
                        ...
                        if st.button("✏️ Editar Objetivo", key=f"btn_edit_obj_toggle_{rol['id']}"):
                    ...
                    if c_h2.button("Añadir", key=f"btn_add_item_{rol['id']}", use_container_width=True) and n_it:
                        st.session_state.data["roles"][idx].setdefault("histeric_list", []).append({"id": str(uuid.uuid4()), "text": n_it})
                        guardar_datos(st.session_state.data)
                        st.rerun()
                    
                    for i_it, it in enumerate(rol.get("histeric_list", [])):
                        c_it1, c_it2 = st.columns([5, 1])
                        c_it1.write(f"- {it['text']}")
                        if c_it2.button("❌", key=f"del_item_{it['id']}"):
                            st.session_state.data["roles"][idx]["histeric_list"].pop(i_it)
                            guardar_datos(st.session_state.data)
                            st.rerun()
                    ...
                    with st.expander("⚠️ Opciones Avanzadas (Eliminar Rol)"):
                        if st.button("🗑️ Eliminar este Rol Completo", type="primary", key=f"del_full_role_{rol['id']}"):
                            st.session_state.data["roles"].pop(idx)
                            guardar_datos(st.session_state.data)
                            st.rerun()